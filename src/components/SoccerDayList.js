...
export const SoccerDayList = ({ days, filter }) => {
  const filteredDays =
    !filter || !filter.match(/grass|turf/)
      ? days
...
SoccerDayList.PropTypes = {
  days: function(props) {
    if (!Array.isArray(props.days)) {
      return new Error("SoccerDayList should be an array");
    } else if (!props.days.length) {
      return new Error("SoccerDayList must have at least one record");
    } else {
      return null;
    }
  }
